        ...
        <div onClick={closeFreshRestPopup} ref={menuRef} className="fixed inset-0 h-full flex-center flex-col bg-black bg-opacity-60 backdrop-blur-sm z-50">
            <div className="flex flex-col gap-3">
                ...
                <div className=" w-full flex flex-col gap-3 p-5  max-w-md bg-black border border-slate-500 rounded-xl">
                    <h2 className='text-2xl font-semibold text-gray-100'>Items already in cart</h2>
                    <p className='text-base font-light text-gray-300'>
                        Your cart contains items from other restaurant. Would you
                        like to reset your cart for adding items from this
                        restaurant?
                    </p>
                    <button onClick={startFresh} className="p-3 bg-slate-900 hover:bg-slate-950 text-white text-lg rounded-lg shadow-lg border border-gray-400" >
                        Yes, Start Fresh
                    </button>
                    <button className="flex-center gap-2 p-3  bg-slate-900 hover:bg-slate-950 text-white text-lg rounded-lg shadow-lg border border-gray-400 " onClick={onClose}>
                        No
                    </button>
                </div>
            </div>
