    ...
    return (
        <div onClick={closeLocationMenu} ref={menuRef} className="fixed inset-0 h-full flex-center flex-col bg-black bg-opacity-60 backdrop-blur-sm z-50">
            <div className="flex flex-col gap-3 max-w-[346.42px] w-full">
                <button onClick={onClose} className="self-end text-2xl bg-slate-900 hover:border-slate-500 p-1 text-white border border-slate-500 rounded-full transition-all duration-300 hover:scale-90 focus:scale-75">
                    <RxCross1 />
                </button>
                <div className=" w-full flex flex-col gap-3 p-5  max-w-md bg-black border border-slate-500 rounded-xl">
                    <button disabled={isFetching} className="text-white flex-center gap-2 p-3  bg-slate-900 hover:bg-slate-950 text-lg rounded-lg shadow-lg border border-gray-400 disabled:bg-slate-700" onClick={() => { handleDetectLocation(null) }}>
                    ...
                    <div className="w-full flex items-center gap-1">
                        ...
                        <span className="text-center select-none text-white font-extralight text-sm">OR</span>
                        <span className="border-t w-full border-dashed border-gray-400 "></span>
                    </div>
                    <button className=" text-white flex-center gap-2 p-3  bg-slate-900 hover:bg-slate-950 text-lg rounded-lg shadow-lg  border-gray-400 border" onClick={() => { handleDetectLocation("default") }}>
                        Default location
                    </button>
                </div>
            </div>
